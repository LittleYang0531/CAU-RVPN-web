...
void exitRequest(client_conn& conn) {
    ...
    if (conn.thread_id != -1) longjmp(buf[conn.thread_id], 0);
}
...
http_request getRequest(client_conn& conn) {
    ...
    if (request.argv.count("content-length") && request.argv["content-length"] != "0") 
        request.postdata = recv(conn, atoi(request.argv["content-length"].c_str()));
